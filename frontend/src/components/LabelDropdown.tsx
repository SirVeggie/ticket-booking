...
export default function LabelDropdown<Item>(p: LabelDropdown<Item>) {
  ...
  return (
    <div style={p.style ? p.style : { margin: '0 10px 10px 0' }}>
      ...
      <Dropdown search selection value={value} compact={!!p.width} style={{ width: p.width ? p.width : undefined }} options={options} onChange={(event, dropdata) => p.update(p.items[Number(dropdata.value)])} />
    </div>
  );
}